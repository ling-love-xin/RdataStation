...
```typescript
import type { ExtensionContext, ExtensionModule, ExtensionAPI, Disposable, ProjectInfo } from '@/extensions/core/types'
...
class ExtensionHost {
  ...
  async activateExtensions(extensions: Array<{ id: string, module: ExtensionModule }>, projectInfo: ProjectInfo) {
...
<template>
  <dockview
    :components="getComponentsFromRegistry()"
