...
| **预期结果** | 1. 每个错误域返回正确的错误码前缀（COMMON_/CONN_/DB_/STORE_/CACHE_）<br>2. category()返回正确枚举<br>3. Timeout/Network/PoolError返回is_retryable=true<br>4. Display包含错误码和描述信息 |
...
| **测试步骤** | 1. DuckDB instance 通过ATTACH注册MySQL<br>2. 执行 SELECT d.*, m.* FROM duckdb_table d JOIN mysql_table m ON d.id=m.id<br>3. 验证结果 |
