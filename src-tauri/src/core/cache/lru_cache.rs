...
use super::{CacheEntry, CacheStats, CachePolicy, CacheKey, CacheValue};
        ...
        if let Ok(mem_info) = std::fs::read_to_string("/proc/meminfo") {
            for line in mem_info.lines() {
                if line.starts_with("MemAvailable:") {
                    if let Some(available_kb) = line.split_whitespace()
                        .nth(1)
        ...
        if let Some(output) = output {
            ...
            for line in stdout.lines() {
                if line.contains("Pages free") {
                    if let Some(pages) = line.split_whitespace()
                        .last()
...
impl<K: CacheKey, V: CacheValue> LruCache<K, V> {
    ...
    pub fn get(&mut self, key: &K) -> Option<V> {
        // 检查是否存在且未过期
        let is_valid = self.map.get(key).map(|entry| !entry.is_expired()).unwrap_or(false);
        ...
        V: MemoryEstimate 
    {
        ...
        let value_size = self.map.values()
