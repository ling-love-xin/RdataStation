...
pub fn get_global_data_dir() -> Result<PathBuf, CoreError> {
    ...
    std::fs::create_dir_all(&app_dir).map_err(|e| CoreError::common(CommonError::General(
        format!("Failed to create global data directory: {}", e),
    )))?;
...
pub fn get_system_dir() -> Result<PathBuf, CoreError> {
    ...
    std::fs::create_dir_all(&system_dir).map_err(|e| CoreError::common(CommonError::General(
...
pub fn get_global_metadata_dir() -> Result<PathBuf, CoreError> {
    ...
    std::fs::create_dir_all(&metadata_dir).map_err(|e| CoreError::common(CommonError::General(
...
pub async fn initialize_global_system() -> Result<(), CoreError> {
    ...
    if !applied.is_empty() {
        tracing::info!("Global system SQLite initialized with {} migrations", applied.len());
    } else {
    ...
    // 存储到全局实例
    GLOBAL_DB_MANAGER.set(manager).map_err(|_| CoreError::common(
        CommonError::General("Global database manager already initialized".to_string())
    ))?;
