...
let currentActiveDialect: SqlDialect = 'generic'
...
// 基础 tokenizer 工厂（注释、字符串、数字、标识符规则）
function buildBaseTokenizerConfig(): monaco.languages.IMonarchLanguage {
  return {
    operators: [
      '=',
      '>',
...
// 构建特定方言的 tokenizer 配置
function buildDialectConfig(dialect: SqlDialect): monaco.languages.IMonarchLanguage {
  const keywords = DIALECT_KEYWORDS[dialect] || []
  const functions = DIALECT_FUNCTIONS[dialect] || []

  const base = buildBaseTokenizerConfig()
  base.keywords = [...BASE_KEYWORDS, ...keywords]
  base.builtinFunctions = functions
  return base
}

export function getCurrentDialect(): SqlDialect {
  return currentActiveDialect
}

export function setCurrentDialect(dialect: SqlDialect): void {
  if (currentActiveDialect === dialect) return
  currentActiveDialect = dialect
  registerDialectHighlight(dialect)
}

/**
 * 注册方言高亮规则（增量：仅重建关键字/函数列表，基础规则复用）
 */
export function registerDialectHighlight(dialect: SqlDialect): monaco.IDisposable {
  if (registeredHighlighters.has(dialect)) {
    const existing = registeredHighlighters.get(dialect)!
    currentActiveDialect = dialect
    return existing
  }

  const config = buildDialectConfig(dialect)
  const disposable = monaco.languages.setMonarchTokensProvider('sql', config)

  registeredHighlighters.set(dialect, disposable)
  currentActiveDialect = dialect
  return disposable
}
