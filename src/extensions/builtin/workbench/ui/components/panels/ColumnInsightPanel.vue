    ...
    <NTabs type="segment" size="small" :default-value="activeTab" animated class="insight-tabs">
      <NTabPane name="column" :tab="t('resultPanel.columnInsight')">
        ...
        <div v-else-if="insightStore.isLoading" class="insight-loading">
          <NSpin size="small" />
          <span class="loading-text">{{ t('resultPanel.analyzing', { column: insightStore.currentColumn }) }}...</span>
        ...
        <div v-else-if="insightStore.insightData" class="insight-content">
          <div class="insight-header">
            <span class="insight-title">{{ t('resultPanel.columnInsight') }}: {{ insightStore.currentColumn }}</span>
            <div class="header-actions">
              <NButton size="tiny" quaternary :title="t('resultPanel.exportJson')" @click="exportJSON">
                <Download :size="11" />
              </NButton>
              <NButton size="tiny" quaternary :title="t('resultPanel.exportMarkdown')" @click="exportMarkdown">
                <FileText :size="11" />
              </NButton>
              <NButton v-if="!insightStore.savedVersionId" size="tiny" quaternary :loading="insightStore.isSaving" @click="insightStore.saveCurrentInsight()">
                <Save :size="11" />
              </NButton>
              <NTag v-else :bordered="false" size="tiny" type="success">{{ t('resultPanel.saved') }}</NTag>
            </div>
          </div>

          <div v-if="insightStore.qualityScore" class="quality-score-section">
            <div class="quality-badge" :class="scoreLevelClass">
              <span class="quality-score-num">{{ Math.round(insightStore.qualityScore.overall_score) }}</span>
            ...
            <div class="stat-grid">
              ...
              <div class="stat-row">
                ...
                <span class="stat-value" :class="{ 'text-warning': insightStore.insightData.stats.null_rate > 0.05 }">
                  ...
                  <span class="stat-sub">({{ t('resultPanel.nullRateDetail', { count: insightStore.insightData.stats.null_count }) }})</span>
                </span>
              </div>
              ...
              <template v-if="insightStore.statsKind === 'DateTime'">
                ...
                <div class="stat-row">
                  ...
                  <span class="stat-value">{{ t('resultPanel.spanDays', { days: (insightStore.insightData.stats.stats_detail as DateTimeStatsDetail).span_days }) }}</span>
                </div>
              </template>
              ...
              <template v-if="insightStore.statsKind === 'Boolean'">
                <div class="stat-row">
                  ...
                  <span class="stat-value">{{ (insightStore.insightData.stats.stats_detail as BooleanStatsDetail).true_count.toLocaleString() }}
                    <span class="stat-sub">({{ t('resultPanel.trueRatio', { ratio: ((insightStore.insightData.stats.stats_detail as BooleanStatsDetail).true_ratio * 100).toFixed(1) }) }})</span>
                  </span>
                </div>
                <div class="stat-row">
...
v-if="insightStore.statsKind === 'Text' && (insightStore.insightData.stats.stats_detail as TextStatsDetail).top_values.length >= 5"
                class="quality-item quality-info">
                ℹ️ {{ t('resultPanel.categoryCount', { count: (insightStore.insightData.stats.stats_detail as TextStatsDetail).top_values.length }) }}
              </div>

              <div
v-if="insightStore.statsKind === 'Boolean' && (insightStore.insightData.stats.stats_detail as BooleanStatsDetail).true_ratio > 0.95"
                class="quality-item quality-info">
                ℹ️ {{ t('resultPanel.highlyImbalanced', { ratio: ((insightStore.insightData.stats.stats_detail as BooleanStatsDetail).true_ratio * 100).toFixed(1) }) }}
              </div>
            </div>
          </NCollapseItem>
      ...
      <div v-if="insightStore.storageStats" class="insight-storage-footer">
        <span class="storage-text">{{ t('resultPanel.storageStats', { snapshots: insightStore.storageStats.total_snapshots, columns: insightStore.storageStats.unique_columns, size: insightStore.storageStats.total_size_display }) }}</span>
        <NButton size="tiny" quaternary :loading="insightStore.isCleaning" class="cleanup-btn" @click="handleCleanup">
      ...
      <div v-if="insightStore.columnRules.length > 0" class="insight-rules-footer">
        ...
        <div class="rules-tags">
          <NTag v-for="rule in insightStore.columnRules" :key="rule.id" size="tiny" :bordered="false" type="info" class="rule-tag">
            {{ rule.name }}
          </NTag>
        </div>
      </div>
      ...
      <NTabPane name="history" :tab="t('resultPanel.history')">
        ...
        <div v-else class="history-content">
          <div class="history-header">
            <span class="history-title">{{ t('resultPanel.versionsCount', { count: insightStore.history.length }) }}</span>
            <NButton v-if="insightStore.diffVersionId" size="tiny" quaternary @click="insightStore.clearDiff()">
          ...
          <div class="history-list">
            ...
            >
              <div class="history-item-main">
                ...
                <NTag :bordered="false" size="tiny" :type="insightStore.diffVersionId === entry.version_id ? 'primary' : 'default'">
          ...
          <div v-if="insightStore.diffData && insightStore.insightData" class="diff-panel">
            ...
            <div class="diff-grid">
              <div class="diff-row">
                ...
                <span class="diff-old">{{ (insightStore.diffData.stats.null_rate * 100).toFixed(1) }}%</span>
                <span class="diff-arrow">→</span>
                <span class="diff-new" :class="diffClass(insightStore.diffData.stats.null_rate, insightStore.insightData.stats.null_rate)">
                  {{ (insightStore.insightData.stats.null_rate * 100).toFixed(1) }}%
                </span>
              </div>
              <div class="diff-row">
                <span class="diff-label">{{ t('resultPanel.totalCountCompare') }}</span>
                <span class="diff-old">{{ insightStore.diffData.stats.total_count.toLocaleString() }}</span>
                <span class="diff-arrow">→</span>
                <span class="diff-new" :class="diffClass(insightStore.diffData.stats.total_count, insightStore.insightData.stats.total_count)">
                  {{ insightStore.insightData.stats.total_count.toLocaleString() }}
                </span>
              </div>
              <div v-if="insightStore.diffData.stats.unique_count != null && insightStore.insightData.stats.unique_count != null" class="diff-row">
                <span class="diff-label">{{ t('resultPanel.uniqueCountCompare') }}</span>
                <span class="diff-old">{{ insightStore.diffData.stats.unique_count.toLocaleString() }}</span>
                <span class="diff-arrow">→</span>
                <span class="diff-new" :class="diffClass(insightStore.diffData.stats.unique_count, insightStore.insightData.stats.unique_count)">
              ...
              <div class="diff-row">
                ...
                <span class="diff-arrow">→</span>
                <span class="diff-new" :class="diffClass(insightStore.diffData.stats.null_count, insightStore.insightData.stats.null_count)">
...
function exportJSON(): void {
  ...
  const blob = new Blob([JSON.stringify(insightStore.insightData, null, 2)], { type: 'application/json' })
...
function handleOpenInsight(event: Event): void {
  const detail = (event as CustomEvent).detail as { column: string; tempTable: string; allColumns?: string[] }
  if (detail?.column && detail?.tempTable) {
    insightStore.loadColumnInsight(detail.tempTable, detail.column)
    insightStore.loadHistory(detail.column)
    if (detail.allColumns && detail.allColumns.length > 0) {
      availableColumns.value = detail.allColumns
    }
  }
}

function handleTableColumnClick(event: Event): void {
  const detail = (event as CustomEvent).detail as {
    column: string; table: string; database: string; schema: string; connId: string
  }
  if (detail?.column && detail?.connId) {
...
onMounted(() => {
  ...
  window.addEventListener('table-column-click', handleTableColumnClick)
...
.sample-val { color: var(--text-primary); font-family: monospace; word-break: break-all; line-height: 1.5; font-size: 10px; }
...
.quality-badge.score-excellent { background: #e6f7e6; color: #1a7a1a; }
.quality-badge.score-good { background: #e6f7ff; color: #1a6db5; }
.quality-badge.score-fair { background: #fff7e6; color: #b57a1a; }
.quality-badge.score-poor { background: #fff2e6; color: #b54a1a; }
.quality-badge.score-bad { background: #ffe6e6; color: #b51a1a; }
.quality-score-num { font-size: 18px; font-weight: 700; line-height: 1; }
.quality-level { font-size: 11px; font-weight: 600; }
.quality-summary { font-size: 10px; color: var(--text-secondary); margin-bottom: 6px; line-height: 1.4; }
.quality-dimensions { display: flex; flex-direction: column; gap: 4px; }
.quality-dim { display: flex; flex-direction: column; gap: 1px; }
.dim-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
}
.dim-name { font-size: 9px; color: var(--text-secondary); }
.dim-score { font-size: 9px; color: var(--text-primary); font-weight: 600; }
.dim-bar-track {
  height: 3px;
  background: var(--border-color);
  border-radius: 2px;
...
.dim-bar-fill.bar-good { background: #52c41a; }
.dim-bar-fill.bar-fair { background: var(--warning-color, #FF7D00); }
.dim-bar-fill.bar-poor { background: #f5222d; }
.dim-detail { font-size: 8px; color: var(--text-tertiary); }
...
.storage-text { font-size: 9px; color: var(--text-tertiary); }
.cleanup-btn { font-size: 9px; }
...
.rules-label { font-size: 9px; color: var(--text-tertiary); flex-shrink: 0; }
.rules-tags { display: flex; flex-wrap: wrap; gap: 3px; }
.rule-tag { cursor: default; font-size: 9px; }
...
.history-title { font-size: 11px; color: var(--text-secondary); }
.history-list {
  flex-shrink: 0;
  max-height: 50%;
  overflow-y: auto;
...
.history-date { font-size: 10px; color: var(--text-secondary); }
...
.diff-label { width: 48px; flex-shrink: 0; color: var(--text-tertiary); font-size: 10px; }
.diff-old { color: var(--text-secondary); font-family: monospace; font-size: 10px; }
.diff-arrow { color: var(--text-tertiary); font-size: 10px; }
.diff-new { font-family: monospace; font-size: 10px; font-weight: 600; }
.diff-increase { color: var(--success-color); }
.diff-decrease { color: var(--danger-color); }
