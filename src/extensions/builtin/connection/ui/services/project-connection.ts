...
export async function createProjectConnection(
  ...
  const r = await invoke<ProjectConnectionResponse>('create_project_connection', {
    input: {
      ...
      schemaName: input.schema_name ?? null,
      username: input.username ?? null,
      password: input.password ?? null,
      options: input.options ?? null,
      tags: input.tags ?? null,
      useDuckdbFed: input.use_duckdb_fed ?? false,
      metadataPath: input.metadata_path ?? null,
      description: input.description ?? null,
      driverId: input.driver_id ?? null,
      environmentId: input.environment_id ?? null,
      authConfigId: input.auth_config_id ?? null,
      authMethod: input.auth_method ?? null,
      networkConfigId: input.network_config_id ?? null,
      driverProperties: input.driver_properties ?? null,
      advancedOptions: input.advanced_options ?? null,
    },
  })
