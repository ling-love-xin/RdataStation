<template>
  <NModal :show="modelValue" preset="card" :style="{ maxWidth: '560px' }" :title="title" @update:show="$emit('update:modelValue', $event)">
      ...
      >
        ...
        <div class="env-mgr-content">
          <div class="env-mgr-header">
            ...
            <span v-else-if="sourceLabel(env.id)" :class="['env-mgr-source-tag', sourceKind(env.id)]">{{ sourceLabel(env.id) }}</span>
          </div>
        ...
        <div class="env-mgr-actions">
          ...
          <NButton v-if="!env.builtin" size="tiny" type="error" text @click="$emit('delete', env.id)">✕</NButton>
        </div>
      </div>
    </div>

    <div class="env-mgr-create-section">
      <NButton v-if="!showCreateForm" size="small" dashed @click="$emit('toggle-create')">+ {{ createLabel }}</NButton>
      <div v-else class="env-create-form">
        ...
        <div class="env-create-row">
          ...
          <NInput :value="newName" size="small" :placeholder="namePlaceholder" @update:value="$emit('update:newName', $event)" />
        </div>
        <div class="env-create-row">
          <label class="env-create-lbl">{{ iconLabel }}</label>
          <NInput :value="newIcon" size="small" placeholder="🟢" style="max-width:80px" @update:value="$emit('update:newIcon', $event)" />
          <label class="env-create-lbl" style="margin-left:12px">{{ colorLabel }}</label>
          <input :value="newColor" type="color" class="color-input" @input="$emit('update:newColor', ($event.target as HTMLInputElement).value)" />
        </div>
        <div class="env-create-row">
          <label class="env-create-lbl">{{ descLabel }}</label>
          <NInput :value="newDesc" size="small" :placeholder="descPlaceholder" @update:value="$emit('update:newDesc', $event)" />
        </div>
        <div class="env-create-row">
          <label class="env-create-lbl">{{ templateLabel }}</label>
          <NSelect :value="newTemplate" size="small" :options="templateOptions" style="flex:1" @update:value="$emit('update:newTemplate', $event)" />
        </div>
...
.env-mgr-source-tag { font-size: var(--font-size-xs); padding: 1px 6px; border-radius: var(--border-radius-sm); font-weight: 500; }
.env-mgr-source-tag.global { background: rgba(137,180,250,0.1); color: #89b4fa; }
.env-mgr-source-tag.project { background: rgba(166,227,161,0.1); color: var(--brand-success); }
.env-mgr-source-tag.snapshot { background: rgba(245,194,231,0.12); color: #cba6f7; font-style: italic; }
...
.env-form-title { font-size: 13px; font-weight: 600; color: var(--brand-accent,#e17055); margin-bottom: 2px; }
...
.color-input { width: 32px; height: 32px; padding: 0; border: 1px solid var(--color-border); border-radius: var(--border-radius-sm); background: transparent; cursor: pointer; box-sizing: border-box; flex-shrink: 0; }
